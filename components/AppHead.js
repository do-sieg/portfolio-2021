import Head from "next/head";

export default function AppHead() {
    return (
        <Head>
            <link rel="preconnect" href="https://fonts.gstatic.com"></link>
            <link href="https://fonts.googleapis.com/css2?family=Kufam:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600;1,700&display=swap" rel="stylesheet"></link>
            <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,300,400;0,500;0,600;0,700;1,300,1,400;1,500;1,600;1,700&display=swap" rel="stylesheet"></link>
            <title>PORTFOLIO V2</title>
        </Head>
    );
}